...
You can start writing assets in `play_dagster/assets/`. The assets are automatically loaded into the Dagster repository as you define them.
...
Tests are in the `play_dagster_tests` directory and you can run tests using `pytest`:

```bash
pytest play_dagster_tests
```
